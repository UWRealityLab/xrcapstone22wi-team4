...
namespace UnityEngine.XR.Interaction.Toolkit
{
    ...
    public partial class XRCartGrabInteractable : XRBaseInteractable
    {
...
namespace UnityEngine.XR.Interaction.Toolkit
{
    ...
    public partial class XRCartGrabInteractable : XRBaseInteractable
    {
        ...
        public override void ProcessInteractable(XRInteractionUpdateOrder.UpdatePhase updatePhase)
        {
            ...
            switch (updatePhase)
            {
                ...
                case XRInteractionUpdateOrder.UpdatePhase.OnBeforeRender:
                    if (isSelected)
                    {
                        Debug.Log("On Before Render update");
                        var interactor = interactorsSelecting[0];
                        // Legacy does not support the Attach Transform position changing after being grabbed
                        // due to depending on the Physics update to compute the world center of mass.
                        if (m_AttachPointCompatibilityMode == AttachPointCompatibilityMode.Default)
        ...
        void UpdateTarget(IXRInteractor interactor, float timeDelta)
        {
            ...
            Debug.Log("Groc: UPDATE TARGET:-----------------------------------------");
            Debug.Log("Groc: raw Target World Position: " + rawTargetWorldPosition.ToString());
            Debug.Log("Groc: raw Target World Rotation: " + rawTargetWorldRotation.ToString());

            float yRotation = rawTargetWorldRotation.eulerAngles.y;
            rawTargetWorldRotation = Quaternion.Euler(0f, yRotation, 0f);
            rawTargetWorldPosition = new Vector3(rawTargetWorldPosition.x, 0f, rawTargetWorldPosition.z);

            Debug.Log("Groc: modified Target World Rotation: " + rawTargetWorldRotation.ToString());
            Debug.Log("Groc: modified Target World Position: " + rawTargetWorldPosition.ToString());
            Debug.Log("Groc: END UPDATE TARGET:-------------------------------------------------------");
        ...
        void PerformKinematicUpdate(XRInteractionUpdateOrder.UpdatePhase updatePhase)
        {
            Debug.Log("Groc: PERFORMING KINEMATIC UPDATE:-----------------------------");
            if (updatePhase == XRInteractionUpdateOrder.UpdatePhase.Fixed)
            {
                if (m_TrackPosition)
                {
                    var position = m_AttachPointCompatibilityMode == AttachPointCompatibilityMode.Default
                        ...
                        : m_TargetWorldPosition - m_Rigidbody.worldCenterOfMass + m_Rigidbody.position;
                    m_Rigidbody.velocity = Vector3.zero;
                    m_Rigidbody.MovePosition(new Vector3(position.x, 0f, position.y));
                    Debug.Log("Groc: Position: " + position);
                }
                ...
                if (m_TrackRotation)
                {
                    ...
                    Debug.Log("Groc: Rotation: " + rotation.ToString());
                }
                Debug.Log("Groc: END KINEMATIC UPDATE:-----------------------------");
            }
        }
